<?php
  ...
  function displaySearch() {
    echo '<form class="form-inline"><div class="form-group"><input class="form-control" type="search" placeholder="Search" aria-label="Search"> <button class="btn btn-outline-success" type="button" id="search-button">Search</button></div></form>';
  }

  function displayTweeterBox() {
    if (array_key_exists('id', $_SESSION) && $_SESSION['id'] > 0) {
      echo '<div class="tweeter-div"><p>Post a Tweeter</p><form class="form"><textarea class="form-control" id="tweeter-post" rows="3"></textarea><button class="btn btn-outline-success" type="button" id="tweeter-button">Tweeter-ing</button></form></div>';
    }
  }
?>