<?php
  ...
  function displayTweets($visibility) {
    ...
    if ($visibility == 'public') {
      ...
      // Find nothing if you are not following anybody
      ...
      echo "<p>Showing results for '".mysqli_real_escape_string($link, $_GET['q'])."':<br>";
      $whereClause = "WHERE tweet LIKE '%".mysqli_real_escape_string($link, $_GET['q'])."%'";

    } else if (is_numeric($visibility)) {

      $userQuery = "SELECT * FROM users WHERE id = ".mysqli_real_escape_string($link, $visibility)." LIMIT 1";
      $userResult = mysqli_query($link, $userQuery);
      $user = mysqli_fetch_assoc($userResult);
      echo "<h2>".mysqli_real_escape_string($link, $user['email'])."'s Tweeters:</h2><br>";

      $whereClause = "WHERE userid = ".mysqli_real_escape_string($link, $visibility);
    ...
    if(mysqli_num_rows($result) == 0) {
      ...
      while($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<div class='tweet'><p><a href='?page=publicprofiles&userid=".$user['id']."'>".$user['email']."</a>   <span class='time'>".convertToTimeAgo($row['datetime'])."</span></p>";
        echo "<p>".$row['tweet']."</p>";
  ...
  function displaySearch() {
    echo '<form class="form-inline">
            <div class="form-group">
              ...
              <button type="submit" class="btn btn-outline-success" id="search-button" type="button">Search</button>
            </div>
          </form>';
  }
  ...
  function displayTweeterBox() {
    if (array_key_exists('id', $_SESSION) && $_SESSION['id'] > 0) {
      echo '<div class="tweeter-div">
              <p>Post a Tweeter</p>
              <div id="tweeter-success" class="alert alert-success">Your tweeter was tweetered successfully.</div>
              <div id="tweeter-error" class="alert alert-danger"></div>
              <form class="form">
                <textarea class="form-control" id="tweeter-post" rows="3"></textarea>
                <button class="btn btn-outline-success" type="button" id="tweeter-button">Tweeter-ing</button>
              </form>
            </div>';
    }
  }
